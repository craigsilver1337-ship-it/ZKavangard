...
def test_zk_security():
    ...
    print('\n[X][X][X][X] Test 1: Generate valid proof')
    valid_request = {
    ...
    print('[OK] Proof generation started, job_id: {job_id}')
        ...
        print(f'[X][X][X] Failed to start proof generation: {e}')
        return

    # Poll for completion
    valid_proof = None
    claim = None
    print('[X][X][X] Waiting for proof generation...')
    for i in range(30):
        time.sleep(1)
        try:
            ...
            if status_result['status'] == 'completed' and status_result['proof']:
                ...
                print(f'[X][X][X] Proof generated in {status_result.get("duration_ms", "?")}ms')
                break
            elif status_result['status'] == 'failed':
                print(f'[X][X][X] Proof generation failed: {status_result.get("error")}')
                return
        except Exception as e:
            print(f'[X][X][X][X][X][X]  Poll attempt {i+1} failed: {e}')

    if not valid_proof:
        print('[X][X][X] Proof generation timeout')
        return

    print(f'\n[X][X][X][X] Proof structure:')
    ...
    print('\n[X][X][X][X] Test 2: Verify valid proof')
    ...
    try:
        ...
        print(f'{"[X][X][X]" if is_valid else "[X][X][X]"} Valid proof verification: {is_valid}')
        print(f'   Duration: {valid_verify_result.get("duration_ms")}ms')
        
        if not is_valid:
            print('[X][X][X] CRITICAL: Valid proof was rejected!')
            return
    except Exception as e:
        print(f'[X][X][X] Verification failed: {e}')
        return

    # Test 3: Fake proof - Modified challenge
    print('\n[X][X][X][X] Test 3: Fake proof - Modified challenge')
    ...
    try:
        ...
        print(f'{"[X][X][X]" if is_rejected else "[X][X][X] SECURITY BREACH"} Modified challenge rejected: {is_rejected}')
    except Exception as e:
        print(f'[X][X][X][X][X][X]  Test failed with error: {e}')

    # Test 4: Fake proof - Modified response
    print('\n[X][X][X][X] Test 4: Fake proof - Modified response')
    ...
    try:
        ...
        print(f'{"[X][X][X]" if is_rejected else "[X][X][X] SECURITY BREACH"} Modified response rejected: {is_rejected}')
    except Exception as e:
        print(f'[X][X][X][X][X][X]  Test failed with error: {e}')

    # Test 5: Fake proof - Modified merkle root
    print('\n[X][X][X][X] Test 5: Fake proof - Modified merkle root')
    ...
    try:
        ...
        print(f'{"[X][X][X]" if is_rejected else "[X][X][X] SECURITY BREACH"} Modified merkle root rejected: {is_rejected}')
    except Exception as e:
        print(f'[X][X][X][X][X][X]  Test failed with error: {e}')

    # Test 6: Wrong claim
    print('\n[X][X][X][X] Test 6: Valid proof with wrong claim')
    fake_verify_4 = {
        'proof': valid_proof,
        'public_inputs': [],
    ...
    try:
        ...
        print(f'{"[X][X][X]" if is_rejected else "[X][X][X] SECURITY BREACH"} Wrong claim rejected: {is_rejected}')
    except Exception as e:
        print(f'[X][X][X][X][X][X]  Test failed with error: {e}')

    # Summary
    print('\n' + '=' * 60)
    print('[X][X][X] ZK Security Test Complete')
