...
export function AgentShowcase() {
  return (
    <div className="glass-strong rounded-3xl p-6 border border-gray-200 dark:border-gray-800 shadow-ios">
      <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">AI Agents</h3>
      <div className="space-y-3">
        {agents.map((agent) => {
          ...
          return (
            <div
              key={agent.id}
              className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all group"
            >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agent.capabilities.map((capability) => (
                      <span
                        key={capability}
                        className="px-2 py-0.5 bg-gray-200 dark:bg-gray-700 rounded-md text-xs text-gray-900 dark:text-gray-100 font-medium"
                      >
                        {capability}
                      </span>
                    ))}
